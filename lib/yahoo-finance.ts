...
function chartQuotesToStockPrices(quotes: YFChartQuote[]): StockPrice[] {
  return quotes
    .filter(d => d.close > 0)
    .map(d => ({
...
export async function getStockPricesFromYahoo(stockCode: string): Promise<StockPrice[]> {
  ...
  try {
    const yf = await getYF()
    const result = await yf.chart(`${stockCode}.T`, {
      period1: from.toISOString().split('T')[0],
      interval: '1d',
    }) as { quotes: YFChartQuote[] }

    return chartQuotesToStockPrices(result.quotes ?? [])
  } catch (e) {
...
export async function getNikkeiPricesFromYahoo(): Promise<StockPrice[]> {
  ...
  try {
    const yf = await getYF()
    const result = await yf.chart('^N225', {
      period1: from.toISOString().split('T')[0],
      interval: '1d',
    }) as { quotes: YFChartQuote[] }
